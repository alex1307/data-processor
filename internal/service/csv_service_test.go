...
func TestFindUniqueIDs(t *testing.T) {
	listing_service := NewGenericCSVReaderService[modelcsv.Listing]()
	listing_filename := "../../resources/test/listing.csv"
	err := listing_service.ReadFromFiles(listing_filename)
	if err != nil {
		t.Errorf("Error: %v", err)
	}
	details_service := NewGenericCSVReaderService[modelcsv.Details]()
...
func TestProcess(t *testing.T) {
	listing_filename := "../../resources/test/listing.csv"
	details_filename := "../../resources/test/details.csv"
	all, err := Process(listing_filename, details_filename)
	if err != nil {
		t.Errorf("Error: %v", err)
	}
	assert.Equal(t, len(all), 67)
	record_service := NewGenericCSVReaderService[modelcsv.Record]()
...
func TestNotFound(t *testing.T) {
	...
	not_found_service := NewGenericCSVReaderService[modelcsv.NotFound]()
